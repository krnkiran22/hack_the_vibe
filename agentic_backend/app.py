...
def chat():
    ...
    if not conversation_id:
        # Generate a unique conversation ID using secrets (Python 3 compatible)
        conversation_id = ''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(32))
    ...
    if conversation_id not in conversation_history:
        ...
        web3_keywords = ['swap', 'trade', 'buy', 'sell', 'send', 'transfer', 'wallet', 'balance', 'pool', 'deepbook']
        is_web3_query = any(k in query.lower() for k in web3_keywords)
        
        if is_web3_query:
            path_to_try = os.path.join(current_path, "web3_agent_prompt.txt")
        else:
        ...
        - Provide detailed tactical advice:
          - "Try using the Shotgun in close-quarters maps like City Side."
          - "On the Water Pokemon map, use the high ground near the waterfall."
          - "Your movement was good, but try jumping less when firing the Sniper to improve stability."
        - Answer any cross-questions about game mechanics, weapons (AK, Sniper, GrenadeLauncher, etc.), or maps confidentially.
        '''
